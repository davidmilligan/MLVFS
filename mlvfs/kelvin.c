...
void ufraw_kelvin_green_to_multipliers(double temperature, double green, double chanMulArray[3], const char *name)
{
    ...
    int black, maximum;
    adobe_coeff(name, &black, &maximum, cam_mul, pre_mul, cmatrix, rgb_cam);
    double rgbWB[3];
    int c, cc, i, j;
...
void CLASS cam_xyz_coeff (double cam_xyz[4][3], float cam_mul[4], float pre_mul[4], float cmatrix[3][4], float rgb_cam[3][4])
{
 ...
 */
void CLASS adobe_coeff (const char *name, int * black, int * maximum, float cam_mul[4], float pre_mul[4], float cmatrix[3][4], float rgb_cam[3][4])
{
    static const struct {
        const char *prefix;
    ...
    for (i=0; i < sizeof table / sizeof *table; i++)
        if (!strncmp (name, table[i].prefix, strlen(table[i].prefix))) {
            ...
            if (table[i].trans[0]) {
                ...
                cam_xyz_coeff (cam_xyz, cam_mul, pre_mul, cmatrix, rgb_cam);
            }
            break;
        }
}